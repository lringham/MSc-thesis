...
Pigments are molecules that provide the colour seen in many natural patterns. Anthocyanins and carotenoids are two examples of pigments found in flowers. The first is responsible for colours such as blue and purple. And the second is responsible for red and yellow. \citet{martin1993} studied the role genes play in how pigments are created in flowers. These pigments are created when the flower petals are developing. A petal starts as primordium and develop into a limb. The limb is derived from two independent rings of cells that form the inner and outer epidermis of the petal. 

As the limb cells are dividing there is a progression of progeny toward the tip of the petal. This results in sectors and lines as cells with pigments reproduce?

 Anthocyanins are found in these epidermal petal cells. 5 to 6 days before the flower has matured, the petals cells have stopped dividing. In the remaining days, cells undergo a process of elongation. Production of pigments occur during this process.
 
 Transcription factors are the main morphogens involved in pattern formation. They are proteins responsible for gene expression and consequently, pigment production.
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=\columnwidth]{monkeyflower.pdf}
...
\textbf{Model a:} 
This model is of a \textit{Phalaenopsis} orchid, which displays varying sizes of purple spots on a white background. The \textit{Phalaenopsis} orchid was a modelling challenge due to the variance in spot size and the relatively complex geometry of the flower. To simulate the purple spots, a regular spot pattern was used and the simulation was before the pattern became fully stable. This allowed for varying sizes of spots in the resulting pattern. The parameters are listed in Table. \ref{tab:flowerAParameters}.
...
\begin{table}[hb]
	...
	\caption {Parameter values for  \textit{Encyclia} and \textit{Kohleria} using Turing reaction-diffusion (Eqn. \ref{eq:turingRD}). $uSat$ represents the maximum value of the $u$.}
...
\begin{table}[H]
	...
	\caption {Parameters for the \textit{Digitalis} model using the activator-depleted substrate formula (Eqn. \ref{eq:activatorSubstrateRD}). This model uses two morphogens $s$ and $a$. The substrate $s$ is $1$ everywhere and $a$ is $0$ except for a few vertices at the bottom of the flower. These cells have a value of $a=1$ and will progress over time to become the dark purple pigment spots.}
