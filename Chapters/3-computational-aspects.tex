...
% reorganization
% 1. Pose problem first in the context of a figure. 
%		Figure needs diffusion dir

% 2. Be as explanatory as possible
% 3. Introduce major elements that come into play
...
In nature, domains grow over time, which can have an appreciable effect on pattern formation. One of them is a diluting effect on chemical concentrations that make up a pattern. When simulating directly on meshes, the face's area represents a minimum size of pattern detail that can be represented. Thus, it is important to consider the increase in face size from growth. To address this, a subdivision algorithm is used to split large triangles into a few smaller ones. Subdividing every face in the mesh can lead to small triangles to be subdivided unnecessarily and the total number of triangles to grow very quickly. This is a problem because the simulation performance decreases as the number of vertices increases. 
