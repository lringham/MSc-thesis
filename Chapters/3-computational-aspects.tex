...
\begin{figure}[H]
	\centering
	\incfig{aniso_vec2}
	\caption[Visualization of a diffusion tensor at a cell and diffusion direction on a face]{Visualization of a diffusion tensor at a cell and diffusion direction on a face. \textbf{a:} the diffusion tensor at a vertex, $i$. The green arrow, $\boldsymbol{\vec{d_i}}$, is the primary direction of diffusion and its length corresponds to the first eigenvalue value of $\Lambda$. The red arrow is perpendicular to $\boldsymbol{\vec{d_i}}$ and its length corresponds to the second eigenvalue value of $\Lambda$. \textbf{b:} A triangle with diffusion vectors at each vertex and the resulting face vector.}
	\label{fig:anisoMesh}
\end{figure}
