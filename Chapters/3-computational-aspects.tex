...
%TODO use another fig to show morphogen gradients?
The morphogens at vertex $j$ have a morphogen gradient direction which is perpendicular to the edge $e_j$ (Fig. \ref{fig:anisoMesh}). To find the gradient direction, we apply a $90^\circ$ rotation matrix, $Q$, about the face normal to $e_j$. $Q$ is multiplied with $D$ to reflect this operation: %TODO understand why morph grad is always perp to the opp edge
\begin{equation} 
	D^\perp = Q^T D Q.
\label{eq:perpTensor}
\end{equation}
...
%TODO define \sim
%TODO add adjacent triangle angles to figure?
%TODO explain the piecewise nature of Laplacian?

This Laplacian is more complicated than before, and a complete derivation is given in \citep{mathieu2014}. $i$ and $j$ are vertices, and $e_i$ and $e_j$ are edges, as shown in Fig. \ref{fig:meshLaplacian}. The face gradient vector is then transformed by the diffusion tensor, $D$. The amount that this vector is transformed determines the amount of the anisotropy. 
