...
\begin{bmatrix}
    ...
    0 & \lambda_2 
\end{bmatrix}\]
Each $\lambda_i$ represents the diffusivity in a basis of the space. Thus, $\Lambda$ is axis-aligned. Arbitrary orientations can be calculated with a rotation matrix $R$, allowing us to represented general anisotropic diffusion:
\begin{equation}
...
Anisotropic diffusion on a mesh requires a discrete cotangent Laplacian that calculates diffusion from a direction. To represent this direction on a mesh, a direction vector, $d$, is assigned to each mesh vertex. Fig \ref{fig:anisoMesh}a shows a green and red arrow who's lengths are $\lambda_1$ and $\lambda_2$ respectively. As shown in Eqn. \ref{eq:anisoDiffTensor}, the diffusion tensor D is used to modify the diffusion gradient. This gradient is only defined per face and $d$ is defined per vertex. To remedy this, for a given face calculate an angle weighted average of the diffusion directions specified at the vertices and project the result onto the plane of the triangle. This is shown in Fig 3b. In order to apply the diffusion tensor to the morphogen gradient, the gradient must be in the same space as the tensor. To transform the gradient we construct the matrix R from the face direction, face normal, and a bivector.
