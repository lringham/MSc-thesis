...
\ProgramName{} was designed to fulfil five main requirements.
...
\begin{enumerate}
	...
	% interactivity
	\item To interact with simulation parameters at runtime. Real-time interaction allows the user to easily gain an intuition about the behaviour of a pattern.

	% real time performance
	%TODO look for more info on user attention span wrt simulation programs
	\item Pattern formation in real-time. This is desirable because rapid iteration facilitates model creation and pattern exploration. Studies have shown that when using websites, a delay greater than 1 second interrupts the users flow of thought and if a delay is greater than 10 seconds, the user will want to do something else \citep{nielsen1994}. Consequently, minimizing delays increases user enjoyment and productivity. 
	
	% easy to use 
	\item To be able to quickly define and edit PDEs. Allowing configurable PDEs greatly increases the usefulness of \ProgramName{} as users can create their own models. Also, not all users may have access to the program's source code. 
	...
	\item The ability to track incremental changes to models. 
