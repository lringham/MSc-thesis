%% Design
%Specific design decisions were made to meet these requirements. 
%
%The morphogen concentrations in the domain are stored separately from their spatial arrangement. These concentrations are the cells of a grid or the vertices of a mesh. Specialized classes can implement the functionality that depends on the domain geometry, making it easy to support multiple domain types and fulfilling the first requirement.
%
%To meet the second requirement, a GUI facilitates interaction with the simulation at runtime. The GUI provides inputs and controls in the form of text boxes and buttons. Allowing the user to update parameter values and pattern visualization settings. GUI interaction is complemented by \Quotes{painting} functionality. Painting allows parameters to be specified directly on the domain instead of through the GUI.
%
%Designing around the GPU satisfies the third requirement. The GPU performs computation in parallel, providing a significant increase in performance. This increased performance allows for continuous visualization of pattern formation over time while still allowing for real-time interaction. 
%
%Parameters and PDEs are specified together in a text file. Text files are ubiquitous for settings files as they are simple to read and edit. During program initialization, the text file is compiled into a simulation module, avoiding the need to edit \ProgramName{}'s source code.
%
...
\begin{enumerate}
	...
	\item To simulate reaction-diffusion on grids and triangle meshes. Grids provide a common and familiar domain for simulating reaction-diffusion, and meshes afford a more realistic domain because they are not restricted to the plane and are better suited for growth. 
	...
	\item Visualization of pattern formation over time. Watching a pattern's development allows the user to gain an intuition about simulation behaviour. Also, pattern seen in nature are not necessarily those at a steady state.
...
\ProgramName{} was implemented on the Windows 10 operating system using C++ and OpenGL. A library named \Quotes{Dear ImGui} \citep{cornut2019} is used to implement the graphical user interface (GUI). A file named \Quotes{imgui.ini} contains the position and sizes of the GUI elements. 
